...
apt-get update
apt-get install -yq ca-certificates git build-essential supervisor ripgrep
...
cat >/etc/supervisor/conf.d/wiki_api.conf << EOF
[program:wiki_api]
directory=/opt/app/wiki_api
command=npm start
autostart=true
autorestart=true
user=nodeapp
